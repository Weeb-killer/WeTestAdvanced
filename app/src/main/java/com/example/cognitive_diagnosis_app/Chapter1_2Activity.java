...
public class Chapter1_2Activity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        QueueDBService QueueService=new QueueDBService();//打开数据库，变量名字为dbService
        final List<question> queuelist=QueueService.getQuestion();//queuelist stack题目
        ...
        question q=queuelist.get(0);//第一个问题
        ...
        btn_next.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(current<count-1){
                    current++;
                    question q=queuelist.get(current);
                    tv_question.setText(q.question);
                    radioButtons[0].setText(q.answerA);
                    radioButtons[1].setText(q.answerB);
                    radioButtons[2].setText(q.answerC);
                    ...
                    new AlertDialog.Builder(Chapter1_2Activity.this).setTitle("提示").setMessage("答题已完成，是否退出？")
                            .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialogInterface, int i) {
                                    Chapter1_2Activity.this.finish();
                                }
                            }).setNegativeButton("取消",null).show();
                }
                else{  //做完了题目但还没进去错题模式
                    final List<Integer> wrongList=checkAnswer(queuelist);
                    ...
                    for(int i=0;i<queuelist.size();i++){
                        if (wrongList.contains(i)){
                            ...
                            values.put("id",queuelist.get(i).ID);
                            values.put("content",queuelist.get(i).question);
                            values.put("option_A",queuelist.get(i).answerA);
                            values.put("option_B",queuelist.get(i).answerB);
                            values.put("option_C",queuelist.get(i).answerC);
                            values.put("option_D",queuelist.get(i).answerD);
                    ...
                    if(wrongList.size()==0){new AlertDialog.Builder(Chapter1_2Activity.this) //全对时
                            .setTitle("提示")
                        ...
                        new AlertDialog.Builder(Chapter1_2Activity.this)
                                .setTitle("提示")
                                .setMessage("您答对了"+(queuelist.size()-wrongList.size())+
                                ...
                                .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialogInterface, int which) {
                                        ...
                                        for(int i=wrongList.size()-2;i< wrongList.size();i++){ //减几就是这个题库的大小
                                            newList.add(queuelist.get(wrongList.get(i)));
                                        }
//
                                        current=0;
                                        count=newList.size();
...
//点击取消时，关闭当前activity
            ...
            public void onCheckedChanged(RadioGroup radioGroup, int checkedId) {
                for (int i = 0; i < 4; i++) {
                    if (radioButtons[i].isChecked() == true) {
                        queuelist.get(current).selectedAnswer = i;
                        break;
                    }
                }
    ...
    private List<Integer> checkAnswer(List<question> queuelist){
        List<Integer> wrongList=new ArrayList<Integer>();
        for (int i=0;i<queuelist.size();i++){
            if (queuelist.get(i).answer!=queuelist.get(i).selectedAnswer){
                wrongList.add(i);
            }
        }
        return wrongList;
